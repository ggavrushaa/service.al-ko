  ...
  if (animItems.length > 0) {
    var animScroll = function animScroll() {
      for (var index = 0; index < animItems.length; index++) {
        ...
        if (scrollY > animItemOffset - animItemPoint && scrollY < animItemOffset + animItemHeight && !animItem.classList.contains('_active')) {
          animItem.classList.add('_active');

          // Start animation number
          if (animItem.querySelectorAll('.js-number-animation').length > 0) {
            animItem.querySelectorAll('.js-number-animation').forEach(function (item) {
              // startAnimationNumber(item);
              animate(item);
              // outNum(+item.dataset.num, item, +item.dataset.time, +item.dataset.step);
            });
          }

          // Start animation show/hide svg group block
          if (document.querySelector('.block-number svg')) {
            animationSvgBlock();
          }
        } else {
